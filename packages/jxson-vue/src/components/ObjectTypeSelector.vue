...
// Define object-related field types that this selector handles
const OBJECT_FIELD_TYPES = FIELD_TYPES.filter(type => 
...
const emit = defineEmits<{
  'update:modelValue': [type: FieldType];
}>();
...
const getDisplayLabel = (type: FieldType) => {
  const found = FIELD_TYPES.find(t => t.value === type);
  return found ? found.label : 'Object';
};
</script>
...
<template>
  <div class="relative" ref="dropdownRef">
    ...
    >
      <div
        v-for="type in OBJECT_FIELD_TYPES"
        :key="type.value"
